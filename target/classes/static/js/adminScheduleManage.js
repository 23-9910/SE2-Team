...
$(document).ready(function() {
    ...
    $('#schedule-form-btn').click(function () {
        ...
        /*if(form.startTime.length === 0 || form.endTime.length === 0 || form.fare.length === 0){
            alert("表单项不能为空！");
            return false;
        }else if(!scheduleFormCheck(form)){
            alert("时间格式有误，请重新修改排片！");
        }else{
            return true;
        }*/
...
//删除
     ...
     * @Info   验证排片表单时间格式
     */
    /*function scheduleFormCheck(form){
        var startTime = form.startTime;
        var endTime = form.endTime;

        var startTimeValue = new Date(Date.parse(startTime.replace("-","/")));
        var endTimeValue = new Date(Date.parse(endTime.replace("-","/")));
        var nowValue = new Date();

        if(startTimeValue < nowValue || endTimeValue < startTimeValue || endTimeValue < nowValue){
            return false;
        }else{
            return true;
        }
